...
export const L = (() => {
    const ko = {
        ...
        toggleButtonNameHintAdvanced: '· 이 프롬프트를 버튼 여러 개에 넣기 → 쉼표로 나열 (예: 약, 중, 강)\n· 약/중/강처럼 하나만 골라 쓰는 버튼 → 이름 앞에 같은 말머리 (예: 강도::약, 강도::중)\n   → "중"을 켜면 "약"은 자동으로 꺼집니다. 말머리는 버튼에 표시되지 않습니다.',
        ...
        toggleGroupSetTitle: '말머리 지정 (같은 말머리 버튼끼리 하나만 켜짐)',
        toggleGroupSetPrompt: '말머리를 입력하세요.\n같은 말머리를 가진 버튼끼리는 하나만 켜집니다. (예: 강도)\n비워두면 이 그룹은 독립적으로 켜고 꺼집니다.',
        toggleGroupHeldByOthers: (names) => `"${names}" 버튼이 아직 켜져 있어서, 같이 쓰는 프롬프트는 꺼지지 않습니다.`,
        toggleGroupInSet: (name) => `말머리 "${name}" — 이 버튼을 켜면 같은 말머리의 다른 버튼은 자동으로 꺼집니다.`,
        toggleGroupSave: '저장',
    ...
    const en = {
        ...
        toggleButtonNameHintAdvanced: '· Put this prompt on several buttons → list names with commas (e.g. Low, Mid, High)\n· Buttons you pick just one of → give them the same prefix (e.g. level::Low, level::Mid)\n   → Turning "Mid" on turns "Low" off automatically. The prefix is not shown on the button.',
        ...
        toggleGroupSetTitle: 'Assign prefix (buttons sharing a prefix keep only one on)',
        toggleGroupSetPrompt: 'Enter a prefix.\nButtons sharing a prefix keep only one of them on. (e.g. level)\nLeave empty to make this group independent.',
        toggleGroupHeldByOthers: (names) => `"${names}" is still on, so the prompts they share stay enabled.`,
        toggleGroupInSet: (name) => `Prefix "${name}" — turning this on turns off other buttons with the same prefix.`,
